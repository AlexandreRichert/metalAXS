---
title: "Questions fréquentes"
description: "Réponses aux problèmes courants rencontrés dans Sanity Studio : publication, surlignage, tags, images et connexion."
sidebarTitle: "FAQ"
---

Vous trouverez ci-dessous les questions les plus fréquentes. Si votre problème n'est pas listé, contactez votre équipe technique en bas de page.

<AccordionGroup>
  <Accordion title="Je ne vois pas mon article sur le site">
    Vérifiez qu'il est **publié** (bouton *Publish*), que la **date de publication** n'est pas dans le futur, et patientez quelques instants (cache).
  </Accordion>

  <Accordion title="Mon surlignage ne fonctionne pas / je ne peux pas publier">
    Le champ **Mot(s) à surligner** doit correspondre **exactement** à une portion du titre. Corrigez l'un ou l'autre, puis republiez.
  </Accordion>

  <Accordion title="Je ne trouve pas le tag dans la liste">
    Les tags sont **pré-configurés** (voir [Tags](/guide/tags)). Si un tag attendu n'apparaît pas, contactez votre équipe technique. N'en créez pas un nouveau sans validation.
  </Accordion>

  <Accordion title="Une image ne s'affiche pas">
    Attendez la fin de l'upload (barre de progression), vérifiez le texte alternatif si besoin, puis **republiez** l'article.
  </Accordion>

  <Accordion title="Mon article n'apparaît pas en vedette">
    L'article vedette est toujours le **plus récent** par date de publication. Ajustez la date si vous souhaitez le mettre en avant (attention : cela change aussi l'ordre dans la liste).
  </Accordion>

  <Accordion title="Les filtres n'affichent pas mon groupe">
    Vérifiez que le groupe et ses tags sont **publiés**, et que l'**ordre d'affichage** est renseigné.
  </Accordion>

  <Accordion title="Je n'arrive pas à me connecter au Studio">
    Vérifiez l'**e-mail d'invitation**. Demandez à votre administrateur de renvoyer l'invitation depuis [manage.sanity.io](https://manage.sanity.io).
  </Accordion>

  <Accordion title="Puis-je revenir en arrière après une erreur ?">
    Oui : **⋮ → Review changes / History** sur le document concerné.
  </Accordion>
</AccordionGroup>

## Besoin d'aide ?

Contactez votre équipe technique en précisant :

- la page concernée ([Studio](https://all-access-metal.vercel.app/studio), [Blog](https://all-access-metal.vercel.app/blog), ou l'URL de l'article)
- ce que vous essayiez de faire
- une capture d'écran si possible
